...
**分層契約圖**：[`docs/diagrams/architecture/backend-layering-and-celery-boundary.html`](../../../docs/diagrams/architecture/backend-layering-and-celery-boundary.html)（issue #670）以圖面呈現本節與 F-12 的責任邊界、依賴方向與 Celery 任務邊界。該圖為衍生視圖，條文以本節與 F-12 為準；圖上標示為「規格未定義」的項目尚未被任何 FR 或約束情境涵蓋，不得當作已核准規則實作。
