...
- [重設密碼 Token 流程：請求 / 寄送 / 驗證 / 設定新密碼](./diagrams/password-reset-token-flow.html)（`diagram_type: sequence`，另由 [004](../004-forgot-reset-password/spec.md) 引用）
...
| 版本 | 日期 | 變更摘要 |
|------|------|---------|
| 1.2.5 | 2026-09-07 | Issue #671：於 流程圖 新增「帳號模組 token 型流程技術序列圖」小節，連結兩張 `archify` `sequence` 圖（`diagrams/auth-token-lifecycle.html`、`diagrams/password-reset-token-flow.html`）與其 `.json` IR 原始檔；圖面內容全數引自 ADR-021、ADR-013 與 Foundation F-04／F-05，未定義處以「規格未定義」標示而不自行裁定。純參考資料補充，未新增、修改或移除任何 FR/AC，走 Lightweight Path。 |
