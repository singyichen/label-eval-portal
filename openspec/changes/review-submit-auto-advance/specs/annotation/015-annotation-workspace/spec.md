...
- **WHEN** 逐項決策中至少一項為 `修正` 或 `無法判定`（依 FR-089 填妥必填理由），使該單位送出後推導為 `爭議中` 而非 `已定稿`，並成功送出審核
...
- **AND**〔定稿豁免〕逐項全數 `通過` 而使該單位推導為 `已定稿` 的送出，MUST 停留於原單位並就地渲染 `ws-review-finalized-card`，MUST NOT 切換至任何其他單位、MUST NOT 發生任何導頁（AC-3.39、FR-053、FR-099 第 7 點）
