...
> **產品檔案（1）**：`design/prototype/pages/task-management/task-detail.data.js`
> **最終群組**：否。
> **相依**：無。本群組是 PR #683 的解鎖前提，須先於群組 2 合併（兩者同檔改動）。

- [ ] 1.1 撰寫 Red 測試覆蓋名冊身分格式（`design/prototype/tests/task-management/issue-688-reviewer-identity.spec.ts`）：四個任務 profile 的 `reviewerIds` 與 `arbiterIds` 元素皆為 `REVIEWER_ROSTER` 之 id 形狀、不含 `@` 字元；審核設定儲存後寫入的值為 id 而非 Email；成員清單仍以 Email 顯示。驗證：執行該檔全數失敗且失敗原因為 seed 仍為 Email 值 [@senior-qa]
- [ ] 1.2 Green：於 `design/prototype/pages/task-management/task-detail.data.js` 將四個任務 profile 的名冊 seed 由 Email 遷移為 `REVIEWER_ROSTER` id，並使審核員比對一律以 id 為鍵、Email 僅保留於顯示欄位。驗證：`PW_PORT=8899 pnpm playwright test tests/task-management/issue-688-reviewer-identity.spec.ts` 全綠 [@senior-frontend]
- [ ] 1.3 執行 `PW_PORT=8899 pnpm playwright test tests/task-management tests/annotation tests/cross-role` 確認既有綠度不退；`XROLE-04`、`XROLE-20`、`XROLE-21` 三個已知落差維持原狀，若任一意外轉綠則停止並回報 [@senior-qa]
